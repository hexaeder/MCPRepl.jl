...
@testset "LSP Integration Tests" begin
    ...
    @testset "Tool Error Handling" begin
        ...
        result2 = goto_def.handler(Dict(
            "file_path" => "/nonexistent/file.jl",
            "line" => 1,
            "column" => 1
        ))
    ...
    @testset "Tool Descriptions" begin
        ...
        for tool in tools
            ...
            elseif tool.name == "lsp_hover_info"
                @test contains(tool.description, "hover") || contains(tool.description, "documentation")
            elseif tool.name == "lsp_document_symbols"
                @test contains(tool.description, "symbols")
            elseif tool.name == "lsp_workspace_symbols"
                @test contains(tool.description, "search") || contains(tool.description, "workspace")
            end
        end
    end
end
